...
//it does this by implementing the CanActivate interface which allows the guard to decide if a route can be activated with the canActivate()
...
// The auth guard uses the authentication service to check if the user is logged in, if they are logged in it returns true from the
//canActivate() method, otherwise it returns false and redirects the user to the login page.
...
export class AuthGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    ...
    console.log("auth guard page:", route, state);
    if (currentUser) {
